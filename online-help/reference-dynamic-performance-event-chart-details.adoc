...
For more information about the dynamic performance thresholds, see link:reference-performance-event-analysis-and-notification.html[What events are]. For information about how Unified Manager ranks the workloads and determines the sort order, see xref:concept-how-unified-manager-determines-the-performance-impact-for-an-incident.adoc[How Unified Manager determines the performance impact for an event].
